        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service) => (
            <Link to={`/services/${service.slug}`}>
              <Card
                key={service.number}
                className="group relative grid grid-cols-1 h-full p-8 bg-card hover:bg-hero-bg transition-all duration-500 border-0 shadow-sm hover:shadow-xl cursor-pointer overflow-hidden"
              >
                {/* Number Background */}
